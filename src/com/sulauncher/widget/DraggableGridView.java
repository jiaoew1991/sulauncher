...
public class DraggableGridView extends ViewGroup implements View.OnTouchListener, View.OnClickListener, View.OnLongClickListener {
    ...
    protected int colCount, childWidth, childHeight, padding, dpi, scroll = 0;
...
//            		lastDelta = 0;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        childWidth = (r - l) / colCount;
        childWidth = Math.round(childWidth * childRatio);
        childHeight = childWidth * 3 / 2;
        padding = ((r - l) - (childWidth * colCount)) / (colCount + 1);
    ...
    {
        int col = getColOrRowFromCoor(x, childWidth), row = getColOrRowFromCoor(y + scroll, childHeight); 
    ...
    protected int getColOrRowFromCoor(int coor, int size)
    {
        coor -= padding;
    ...
    {
        if (getColOrRowFromCoor(y + scroll, childHeight) == -1) //touch is between rows
            return -1;
    ...
    protected Point getCoorFromIndex(int index)
    {
        ...
        return new Point(padding + (childWidth + padding) * col,
                         padding + (childHeight + padding) * row - scroll);
    }
           ...
           switch (action & MotionEvent.ACTION_MASK) {
               ...
               case MotionEvent.ACTION_MOVE:
                   ...
                   if (dragged != -1)
                   {
                       ...
                       getChildAt(dragged).layout(l, t, l + (childWidth * 3 / 2), t + (childHeight * 3 / 2));
...
//                	   clampScroll();
                	   if (deltaX < -50) {
                		   if (onFlingListener != null) {
                			   onFlingListener.onFling(this, event);
                		   }
                	   }
...
//                	   onLayout(true, getLeft(), getTop(), getRight(), getBottom());
    ...
    {
    	View v = getChildAt(dragged);
    	int x = getCoorFromIndex(dragged).x + childWidth / 2, y = getCoorFromIndex(dragged).y + childHeight / 2;
        int l = x - (3 * childWidth / 4), t = y - (3 * childHeight / 4);
    	v.layout(l, t, l + (childWidth * 3 / 2), t + (childHeight * 3 / 2));
    	AnimationSet animSet = new AnimationSet(true);
		ScaleAnimation scale = new ScaleAnimation(.667f, 1, .667f, 1, childWidth * 3 / 4, childHeight * 3 / 4);
    ...
    {
    	int rowCount = (int)Math.ceil((double)getChildCount()/colCount), max = rowCount * childHeight + (rowCount + 1) * padding - getHeight();
    	return max;
    }
